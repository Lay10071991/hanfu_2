      ...
      <table>
        ...
        <tbody>
          <tr v-for="(item, index) in items" :key="item.id">
            ...
            <td>{{ item.name }}</td>
            <td>{{ item.period }}</td>
            <td>{{ item.features ? item.features.split('|').slice(0, 2).join(', ') : '-' }}</td>
            <td>
    ...
    <div v-if="showDialog" class="modal" @click.self="closeDialog">
      <div class="modal-content">
        ...
        <form @submit.prevent="saveItem">
          ...
          <div class="form-group">
            <label>时期</label>
            <input v-model="form.period" placeholder="如: 618-907年" required />
          </div>
          <div class="form-group">
            <label>形制描述</label>
          ...
          <div class="form-group">
            <label>形制特点(用|分隔)</label>
            <textarea v-model="form.features" rows="3" placeholder="特点1|特点2|特点3"></textarea>
          </div>
          <div class="form-group">
            <label>形制图片</label>
...
export default {
  ...
  methods: {
    async loadItems() {
      try {
        const response = await fetch('http://localhost:8082/api/dynasty-shape')
    ...
    async saveItem() {
      try {
        const url = this.isEdit 
          ? `http://localhost:8082/api/dynasty-shape/${this.form.id}`
          : 'http://localhost:8082/api/dynasty-shape'
    ...
    async deleteItem(id) {
      if (confirm('确定要删除这个朝代形制吗？')) {
        try {
          await fetch(`http://localhost:8082/api/dynasty-shape/${id}`, {
            method: 'DELETE'
          })
          this.loadItems()
        } catch (error) {
